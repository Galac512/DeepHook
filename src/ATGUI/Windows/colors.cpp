...
void Colors::RenderWindow()
{
	...
	ColorListVar colors[] = {
		...
		{ "ESP - Player Info (Enemy)", &Settings::ESP::enemyInfoColor },		{ "ESP - Danger Zone: Weapon Upgrade", &Settings::ESP::DangerZone::upgradeColor },
	...
	if (ImGui::Begin(XORSTR("Colors"), &Colors::showWindow, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoResize))
	{
		...
		ImGui::NextColumn();
		{
			if (colors[colorSelected].type == ColorListVar::HEALTHCOLORVAR_TYPE)
			{
				...
				ImGui::PushItemWidth(-1);
				ImGui::SliderFloat(XORSTR("##RAINBOWSPEED"), &colors[colorSelected].healthColorVarPtr->rainbowSpeed, 0.f, 1.f, XORSTR("Rainbow Speed: %0.3f"));
				ImGui::PopItemWidth();
			}
			else
			{
				...
				ImGui::PushItemWidth(-1);
				ImGui::SliderFloat(XORSTR("##RAINBOWSPEED"), &colors[colorSelected].colorVarPtr->rainbowSpeed, 0.f, 1.f, XORSTR("Rainbow Speed: %0.3f"));
				ImGui::PopItemWidth();
			}
		}
		ImGui::End();
