        ...
        ImGui::NextColumn();
        {
                ImGui::BeginChild(XORSTR("Child2"), ImVec2(0, 0), true);
                {
                        ...
                        ImGui::Columns(2, nullptr, false);
                        {
                                ...
                                if (ImGui::Checkbox(XORSTR("AirStuck"), &Settings::Airstuck::enabled))
				{
					if (!ValveDSCheck::forceUT && ((*csGameRules) && (*csGameRules)->IsValveDS()) && Settings::Airstuck::enabled)
					{
						Settings::Airstuck::enabled = false;
						ImGui::OpenPopup(XORSTR("Error###UNTRUSTED_FEATURE"));
					}
				}
