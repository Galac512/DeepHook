...
static void DrawBox( ImColor color, int x, int y, int w, int h, C_BaseEntity* entity )
{
        ...
        else if ( Settings::ESP::Boxes::type == BoxType::HITBOXES )
        {
                static std::map<int, long> playerDrawTimes;
                if ( playerDrawTimes.find( entity->GetIndex() ) == playerDrawTimes.end() )
                {
                ...
                if ( diffTime >= 12 ) {
                        for ( int i = 0; i < set->numhitboxes; i++ ) {
                                ...
                                debugOverlay->DrawPill( vMin, vMax, hitbox->radius, (int)(color.Value.x*255.f), (int)(color.Value.y*255.f), (int)(color.Value.z*255.f), (int)(color.Value.w*255.f), 0.025f );
                        }
                        playerDrawTimes[entity->GetIndex()] = Util::GetEpochTime();
                }
        }
